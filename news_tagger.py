...
# 嚴格 ticker 格式：必須被半/全形括號/方括號/書名號包圍，或接 -TWO / -TW / -US / -HK / -SS / -SZ 後綴。
# 避免新聞裡的「8176 模式」「成交量3049.75億」「2030 年」這種裸 4 碼數字被誤判為 ticker。
# 涵蓋：
#   - 個股：(2330) (2330-TW) [2330] 【2412】 2330-TW 2412-TWO （2330-TW）全形
#   - ETF：00981A-TW（5 碼 + 字母，但 ETF 不在 manifest 內，這裡僅 regex 容忍）
# 後綴白名單寫死避免 "(2330-TWN)" 這種 typo 誤判；後綴後加 (?![A-Z]) 避免「-TW」貪婪
# 吞掉「-TWN」「-TWO」混淆（alternation 以最長 TWO 優先擺前面，配合 negative lookahead）。
_NUMERIC_TICKER = re.compile(
    r'(?:[\(（\[【]|^|(?<=\s))'             # 左：半/全形括號 + 【 + 行首 + 空白
    r'(\d{4,5}[A-Z]?)'                       # 4-5 碼數字 + 可選字母
    r'(?:[\)）\]】]|-(?:TWO|TW|US|HK|SS|SZ)(?![A-Z]))'  # 右：括號 或 -<白名單後綴 + 後綴後非字母>
)
...
AMBIGUOUS_NAMES = {
    ...
    # 精/精緻 常見泛詞（精金屬、精誠）
    "精金", "精誠",
    # 新 X 通用前綴（新建房屋、新產能、新光集團、新興市場）—— 跟 2516/2850/2855/2515 等撞
    "新建", "新產", "新光", "新興", "新東", "新天地",
    # 海力士（韓國 SK Hynix）會跟 4923 力士工業 撞；其他「力 X」通用
    "力士", "力鴻", "力新",
    # 「建設」/「開發」常見泛詞片段（中華開發、新開發）—— 太短時要加
    "開發", "建設",
    # 「國 X」通用前綴（國產替代、國民教育、國際情勢、國民黨）—— 跟 2504 國產 等撞
    "國產", "國民", "國際", "國光", "國巨",
    # 「大 X」通用前綴（大眾運輸、大同小異、大成功）—— 跟 1410 大眾 等撞
    "大眾", "大田", "大江",
    # 「亞 X」通用前綴（亞洲開發、亞太地區）
    "亞洲", "亞太",
}
